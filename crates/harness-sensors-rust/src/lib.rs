...
impl CargoCheck {
    pub fn new() -> Self { Self { id: "cargo-check".into() } }
}
...
impl Sensor for CargoCheck {
    ...
    async fn observe(&self, _action: &Action, world: &World) -> Result<Vec<Signal>, SensorError> {
        let out = world
            ...
            .await
            .map_err(|e| SensorError::Failed { id: self.id.clone(), reason: e.to_string() })?;
...
impl Clippy {
    pub fn new() -> Self { Self { id: "clippy".into() } }
}
...
impl Sensor for Clippy {
    fn id(&self)    -> &SensorId  { &self.id }
    fn kind(&self)  -> Execution  { Execution::Computational }
    fn stage(&self) -> Stage      { Stage::PreCommit }
    ...
    async fn observe(&self, _action: &Action, world: &World) -> Result<Vec<Signal>, SensorError> {
        let out = world
            ...
            .exec(
                "cargo",
                &["clippy", "--message-format=json", "--quiet", "--", "-D", "warnings"],
                Some(world.repo.root.as_path()),
            )
            .await
            .map_err(|e| SensorError::Failed { id: self.id.clone(), reason: e.to_string() })?;
...
fn parse_cargo_messages(stdout: &str, origin: &str) -> Vec<Signal> {
    ...
    for line in stdout.lines() {
        ...
        let primary = diag.spans.iter().find(|s| s.is_primary).or_else(|| diag.spans.first());
        ...
        let code_str = diag.code.as_ref().map(|c| format!(" [{}]", c.code)).unwrap_or_default();
...
fn build_hint(message: &str, level: &str) -> String {
    match level {
        "error"   => format!("Fix this compilation error: {message}. Edit the file and re-run cargo check."),
