    ...
    async fn search(
        ...
        let anchors: Vec<(i64, String)> = if cjk >= 3 {
            let mut stmt = conn
                ...
                .map_err(|e| RecallError::Backend(e.to_string()))?;
            stmt.query_map(params![query, owner, (limit * 5) as i64], |r| {
                Ok((r.get(0)?, r.get(1)?))
            })
            .map_err(|e| RecallError::Backend(e.to_string()))?
            .collect::<rusqlite::Result<Vec<_>>>()
            ...
            let fts: rusqlite::Result<Vec<(i64, String)>> = (|| {
                ...
                let v = stmt
                    .query_map(params![query, owner, (limit * 5) as i64], |r| {
                        Ok((r.get(0)?, r.get(1)?))
                    })?
                    .collect::<rusqlite::Result<Vec<_>>>()?;
                Ok(v)
            ...
            if v.is_empty() {
                ...
                let mut stmt = conn
                    ...
                    .map_err(|e| RecallError::Backend(e.to_string()))?;
                stmt.query_map(params![owner, like, (limit * 5) as i64], |r| {
                    Ok((r.get(0)?, r.get(1)?))
                })
                .map_err(|e| RecallError::Backend(e.to_string()))?
                .collect::<rusqlite::Result<Vec<_>>>()
        ...
        for (anchor_id, session_id) in anchors {
            ...
            let Some(meta) = Self::meta_of(&conn, owner, &session_id) else {
                continue;
            };
            let around =
                Self::read_window(&conn, owner, &session_id, anchor_id - 5, anchor_id + 5)?;
...
mod tests {
    ...
    async fn bookends_correct_for_non_first_session() {
        ...
        r.ensure_session("u1", "a", &SessionMeta::new("a", 1))
            .await
            .unwrap();
        for i in 0..3 {
            r.append(
                "u1",
                "a",
                &RecallMessage::new("user", format!("a-msg-{i}"), i),
            )
            .await
            .unwrap();
        }
        // Session B: 4 messages → global ids 4,5,6,7 (NOT 1-based)
        r.ensure_session("u1", "b", &SessionMeta::new("b", 10))
            .await
            .unwrap();
        r.append(
            "u1",
            "b",
            &RecallMessage::new("user", "find the kraken deployment runbook", 10),
        )
        .await
        .unwrap();
        r.append("u1", "b", &RecallMessage::new("assistant", "second b", 11))
            .await
            .unwrap();
        r.append("u1", "b", &RecallMessage::new("assistant", "third b", 12))
            .await
            .unwrap();
        r.append(
            "u1",
            "b",
            &RecallMessage::new("assistant", "fourth b kraken", 13),
        )
        .await
        .unwrap();
        ...
        assert!(
            !h.bookend_start.is_empty(),
            "bookend_start empty — the global-id bug"
        );
        assert!(
            !h.bookend_end.is_empty(),
            "bookend_end empty — the global-id bug"
        );
        assert!(
            h.bookend_start
                .iter()
                .any(|m| m.content == "find the kraken deployment runbook"),
