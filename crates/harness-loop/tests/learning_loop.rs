...
fn task() -> harness_core::Task {
    harness_core::Task {
        description: "do real work".into(),
        source: None,
        deadline: None,
    }
}
...
async fn review_writes_a_skill_after_enough_tool_calls() {
    let dir = skills_dir();
    let review_model: Arc<dyn Model> = Arc::new(ReviewModel {
    ...
    let loop_ = AgentLoop::new(MainModel {
        turn: AtomicU32::new(0),
    })
    ...
    assert!(
        dir.join("learned-skill").join("SKILL.md").exists(),
        "review should have written the skill"
    );
...
async fn below_threshold_does_not_review() {
    ...
    assert!(
        !dir.join("learned-skill").exists(),
        "no review below threshold"
    );
    let _ = std::fs::remove_dir_all(&dir);
}
