...
use crate::loop_engine::budget::{BudgetLimit, BudgetState};
use crate::loop_engine::level::{GateDecision, HumanGate, LoopLevel, ProposedAction};
use crate::loop_engine::spec::LoopSpec;
use crate::sandbox::{NullSandbox, Sandbox};
use crate::{Subagent, SubagentReport, SubagentSpec};
...
impl LoopEngine {
    ...
    pub fn new(spec: LoopSpec, model: Arc<dyn Model>) -> Self {
        let gate = crate::loop_engine::level::default_gate_for(spec.level);
        Self {
            spec,
            model,
            maker_tools: Vec::new(),
...
mod tests {
    use super::*;
    use crate::loop_engine::{AllowlistGate, TokenBudget};
    use async_trait::async_trait;
