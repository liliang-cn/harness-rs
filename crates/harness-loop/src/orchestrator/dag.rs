...
use crate::orchestrator::job::{Job, JobId, JobState};
...
impl Dag {
    ...
    pub fn succeeded_results(&self) -> Vec<(JobId, crate::orchestrator::job::JobResult)> {
        self.jobs
            .values()
    ...
    ///
    /// What a [`Next::Goto`](crate::orchestrator::Next) has to re-run: sending the graph back
...
/// What a [`crate::orchestrator::Planner`] returns when asked to (re)plan: either more
