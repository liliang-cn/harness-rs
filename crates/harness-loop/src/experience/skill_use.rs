...
//!
//! [`crate::experience::SkillReviser`] needs to know that a bad run was following skill X.
...
//! event to hook — unlike tools, which [`crate::experience::ToolTrace`] can capture from
//! `PreToolUse`.
//!
//! So this is deliberately thin: a shared buffer with the same
//! `snapshot`/`drain` shape as [`crate::experience::ToolTrace`], filled either directly by
