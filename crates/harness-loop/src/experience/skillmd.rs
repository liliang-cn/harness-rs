...
//! pass `harness_context::skills::validate` + `lint`: every spec rule (name regex, 1024-
...
/// Shortest description that clears `harness_context::skills::lint`'s R1 (≥30) *and* R4
...
/// Routing cues `harness_context::skills::lint` R2 accepts. That list lives as a
...
/// Render `---\n<yaml>---\n<body>\n`, the exact shape
/// `harness_context::skills::write_skill_md` round-trips.
...
/// Split a SKILL.md into manifest + body. Mirrors `harness_context::skills::loader`'s
...
///
/// Info findings are tolerated (they're style nudges; `harness_context::skills`' own
...
pub(crate) fn validate_and_lint(m: &SkillManifest, body: &str) -> Result<(), Vec<String>> {
    if let Err(e) = harness_context::skills::validate(m) {
        return Err(vec![e.to_string()]);
    }
    let candidate =
        harness_context::skills::FileSkill::new(m.clone(), body.to_string(), Vec::new());
    let complaints: Vec<String> = harness_context::skills::lint_skills(&[candidate])
        .into_iter()
...
mod tests {
    ...
    fn slugify_produces_spec_legal_names() {
        ...
        for input in [
            ...
            let s = slugify(input);
            assert!(
                harness_context::skills::validate_name(&s).is_ok(),
