...
pub struct RevisionLedger {
    ...
    /// [`crate::experience::SkillDistiller`]; `None` = hand-written or from elsewhere.
...
impl RevisionRequest {
    ...
    pub fn from_dir(
        ...
        harness_context::skills::validate_name(skill_name)?;
...
mod tests {
    ...
    async fn revision_round_trips_through_write_skill_md() {
        ...
        harness_context::skills::write_skill_md(&root, "deploy-runbook", &skill_md(None)).unwrap();
        ...
        let loaded = harness_context::skills::load_skill_dir(&root.join("deploy-runbook")).unwrap();
