...
use crate::experience::episode::Episode;
use crate::experience::llm;
use crate::experience::revise::RevisionLedger;
use crate::experience::skillmd;
...
pub fn existing_skills_in(root: &Path) -> Vec<ExistingSkill> {
    ...
    match harness_context::skills::scan_skills_root(root) {
        Ok(skills) => skills
            .iter()
...
///
/// Guaranteed by construction to pass `harness_context::skills::validate` and to produce
...
impl SkillDraft {
    ...
    /// Re-check the draft against `harness_context::skills`' validator and linter.
    ...
    ///
    /// Delegates to `harness_context::skills::write_skill_md`, which validates by loading
    ...
    pub fn write_to(&self, skills_root: &Path) -> Result<PathBuf, SkillError> {
        harness_context::skills::write_skill_md(skills_root, &self.name, &self.to_skill_md())
    }
}
...
/// SKILL.md — see [`crate::experience::skillmd`] for why that inversion is what makes the
...
fn assemble_draft(ep: &Episode, json: &serde_json::Value) -> Result<SkillDraft, DistillError> {
    ...
    metadata.insert(
        crate::experience::revise::EXPERIENCE_METADATA_KEY.to_string(),
...
mod tests {
    ...
    async fn distilled_draft_passes_validate_and_lint() {
        ...
        harness_context::skills::validate_name(&draft.name).unwrap();

        // Straight through this workspace's own validator + linter.
        let manifest = draft.manifest();
        harness_context::skills::validate(&manifest).expect("validate");
        let findings =
            harness_context::skills::lint_skills(&[harness_context::skills::FileSkill::new(
                manifest,
                draft.body.clone(),
                Vec::new(),
            )]);
        let loud: Vec<_> = findings
            .iter()
            .filter(|f| f.severity != harness_context::skills::LintSeverity::Info)
            .collect();
    ...
    async fn draft_round_trips_through_write_skill_md() {
        ...
        let loaded = harness_context::skills::load_skill_dir(&root.join(&draft.name)).unwrap();
