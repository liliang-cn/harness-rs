...
pub enum SessionEvent {
    Start       { ts_ms: i64, source: String },
    PreModel    { ts_ms: i64, history_len: usize, tools_count: usize },
    PostModel   { ts_ms: i64, output: ModelOutput },
    PreTool     { ts_ms: i64, action: Action },
    PostTool    { ts_ms: i64, call_id: String, result: ToolResult },
    Sensor      { ts_ms: i64, id: String, signals: usize },
    PreCompact  { ts_ms: i64, stage: CompactionStage },
    PostCompact { ts_ms: i64, stage: CompactionStage },
    Heartbeat   { ts_ms: i64, iter: u32 },
    End         { ts_ms: i64 },
}
...
impl Hook for SessionRecorder {
    fn name(&self) -> &str { "session-recorder" }
...
impl SessionStats {
    pub fn from(events: &[SessionEvent]) -> Self {
        let mut s = Self { events: events.len(), ..Default::default() };
...
mod tests {
    ...
    fn sample_log() -> Vec<SessionEvent> {
        vec![
            SessionEvent::Start { ts_ms: 0, source: "Startup".into() },
            SessionEvent::Heartbeat { ts_ms: 1, iter: 0 },
            SessionEvent::PreModel { ts_ms: 2, history_len: 1, tools_count: 3 },
            SessionEvent::PostModel { ts_ms: 100, output: ModelOutput {
                text: Some("hi".into()),
                tool_calls: Vec::new(),
    ...
    fn round_trip_via_serde() {
        ...
        assert!(matches!(parsed[3], SessionEvent::PostModel { ref output, .. } if output.text.as_deref() == Some("hi")));
    }
}
...
pub fn format_event_short(e: &SessionEvent) -> String {
    match e {
        ...
        SessionEvent::PreModel { history_len, tools_count, .. } => {
            format!("→ model (history={history_len}, tools={tools_count})")
        }
        SessionEvent::PostModel { output, .. } => {
            let calls = output.tool_calls.len();
            let txt = output.text.as_deref().unwrap_or("").chars().take(60).collect::<String>();
            if calls > 0 {
                format!(
        ...
        SessionEvent::PreTool { action, .. } => format!("  → tool {} args={}", action.tool, action.args),
        SessionEvent::PostTool { call_id, result, .. } => {
