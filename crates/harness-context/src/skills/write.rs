...
pub fn write_skill_md(dir: &Path, name: &str, content: &str) -> Result<PathBuf, SkillError> {
    crate::skills::validate::validate_name(name)?;
    ...
    let loaded_ok = crate::skills::loader::load(&skill_dir)
        .and_then(|fs| crate::skills::validate::validate(fs.manifest()).map(|_| ()));
    match loaded_ok {
        Ok(()) => Ok(path),
        Err(e) => {
            match prior {
...
mod tests {
    ...
    fn write_then_load_roundtrips() {
        ...
        assert!(p.exists());
        let loaded = crate::skills::loader::load(&dir.join("deploy-runbook")).unwrap();
