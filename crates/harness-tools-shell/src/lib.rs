...
fn check_safe_args(program: &str, args: &[String]) -> Result<(), String> {
    match program {
        "cargo" => match args.first().map(String::as_str) {
            Some(
                "check" | "test" | "build" | "fmt" | "clippy" | "doc" | "tree" | "metadata"
                | "search" | "audit" | "deny" | "outdated" | "bench" | "nextest" | "vendor",
            ) => Ok(()),
            Some(
                "install" | "uninstall" | "publish" | "yank" | "owner" | "login" | "logout"
                | "package",
        ...
        "git" => match args.first().map(String::as_str) {
            Some(
                ...
                | "describe" | "branch" | "remote" | "config" | "shortlog" | "tag",
            ) => {
        ...
        "npm" | "pnpm" | "yarn" | "bun" => match args.first().map(String::as_str) {
            Some(
                "ls" | "list" | "view" | "info" | "config" | "outdated" | "audit" | "doctor"
                | "search" | "ping" | "whoami" | "--version" | "-v",
            ) => Ok(()),
            Some(
                "install" | "i" | "add" | "uninstall" | "remove" | "rm" | "publish" | "pack"
                | "run" | "exec" | "dlx" | "create" | "link" | "unlink" | "version" | "deprecate"
                | "owner" | "login" | "logout" | "init",
            ) => Err(format!(
                "`{program} {}` mutates state; use shell_exec",
                args[0]
            )),
        ...
        "python" | "python3" | "uv" | "pip" | "pip3" => match args.first().map(String::as_str) {
            ...
            Some(
                "list" | "show" | "check" | "freeze" | "config" | "search" | "index" | "--help",
            ) if program.starts_with("pip") => {
                if args[0] == "config"
                    && args
                        .iter()
                        .skip(1)
                        .any(|a| matches!(a.as_str(), "set" | "unset" | "edit"))
                {
        ...
        "node" | "deno" => match args.first().map(String::as_str) {
            Some("--version" | "-v") => Ok(()),
            _ => Err(format!(
                "`{program}` evaluates arbitrary code — use shell_exec"
            )),
        },
            ...
            Some("version" | "env" | "list" | "vet" | "doc" | "fmt" | "mod") => {
                if args[0] == "mod"
                    && args.iter().skip(1).any(|a| {
                        matches!(a.as_str(), "init" | "tidy" | "edit" | "download" | "vendor")
                    })
                {
        ...
        "docker" | "podman" | "kubectl" => match args.first().map(String::as_str) {
            ...
            Some(
                "ps" | "images" | "version" | "info" | "history" | "inspect" | "logs" | "stats"
                | "top" | "port" | "diff" | "search",
            ) => Ok(()),
            Some(
                "get" | "describe" | "explain" | "config" | "api-resources" | "api-versions"
                | "cluster-info" | "events",
            ) if program == "kubectl" => {
                if args[0] == "config"
                    && args.iter().skip(1).any(|a| {
        ...
        "ls" | "pwd" | "rustc" | "rustup" | "rg" | "fd" | "wc" | "head" | "tail" | "cat"
        | "grep" | "tree" | "stat" | "file" | "du" | "df" | "ps" | "uname" | "hostname"
        | "date" | "env" | "which" | "whereis" => {
            // Block xargs-style execution hand-offs.
            if args
                .iter()
                .any(|a| a.contains("-exec") || a.contains("--exec"))
            {
                Err(format!(
                    "`{program}` with -exec is not allowed via shell_read"
                ))
            } else {
                Ok(())
            }
...
const READ_PROGRAMS: &[&str] = &[
    "cargo", "git", "ls", "pwd", "rustc", "rustup", "rg", "fd", "wc", "find", "head", "tail",
    "cat", "grep", "tree", "stat", "file", "du", "df", "ps", "uname", "hostname", "date", "env",
    "which", "whereis", "npm", "pnpm", "yarn", "bun", "python", "python3", "uv", "pip", "pip3",
    "node", "deno", "go", "make", "docker", "podman", "kubectl",
];
...
impl Tool for ShellRead {
    ...
    async fn invoke(
        ...
        let a: ShellArgs = serde_json::from_value(args).map_err(|e| ToolError::InvalidArgs {
            name: self.name().into(),
            reason: e.to_string(),
        })?;
        check_safe_args(&a.program, &a.args).map_err(ToolError::Permission)?;
        run(&a, world).await
    }
}
...
mod tests {
    ...
    fn git_config_read_vs_write() {
        ...
        assert!(check_safe_args("git", &args(&["config", "user.email", "evil@x"])).is_err());
