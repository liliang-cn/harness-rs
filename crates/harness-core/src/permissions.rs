...
//!
//! ```ignore
//! use crate::permissions::{PermissionMode, PermissionRules, PermissionHook};
//! use std::sync::Arc;
//!
...
use crate::{Action, Event, Hook, HookOutcome, Tool, ToolRisk, World};
    ...
    fn decide(&self, action: &Action) -> HookOutcome {
        ...
        match self.rules.mode {
            ...
            PermissionMode::Plan => match self.rules.risk_map.get(name) {
                ...
                // No catch-all, and that is the point. `ToolRisk` is
                // `#[non_exhaustive]`, so from outside this crate a wildcard
                // arm was mandatory and a new risk level would have been
                // silently *allowed* in plan mode — the one place that is
                // supposed to be cautious. In here the attribute does not
                // apply, so adding a variant fails to compile until somebody
                // says what plan mode does with it.
