...
│   ├── harness-orchestrator/      # 单机异步 Run: 并发 Job DAG + 重试 + 重规划 + 可恢复 (§11.6)
...
## 11.6 Orchestrator — 单机异步 Run 编排 (`harness-orchestrator`)

> 一个 **Run** = 一个用户目标,被拆成一张 **Job 的 DAG**。Job 之间声明依赖、
> 依赖满足即**并发**执行、失败按策略**重试/退避/死信**、状态**持久化**故可崩溃续跑。
> 这是 Agent 系统的"任务编排"那一半,**刻意保持单机**(无 Kafka / 无 worker 池 /
> 无分布式锁,只用 `tokio` + 一个状态 store),对齐 DESIGN §2 Non-Goal。

三个循环 crate 的分工:`harness-loop` 跑*单个* agent;`harness-loop-engine` *治理*一个
*循环*;`harness-orchestrator` 把一个目标**扇出**成多个并发、有依赖的 Job。

### 双层状态机(对齐分布式任务系统,但单机落地)

```rust
pub enum RunState { Created, Planning, Executing, Waiting, Aggregating, Completed, Failed, Cancelled }
pub enum JobState { Pending, Queued, Running, Succeeded, Failed, Retrying, DeadLettered, Cancelled }
```

`RunStore` 在**每次状态迁移后**持久化整个 Run;`Orchestrator::resume(run_id)` 把崩溃时
卡在 `Running/Queued/Retrying` 的 Job 复位为 `Pending`,带着已 `Succeeded` 的结果续跑。

### 动态重规划——把 workflow 变回 agent

```rust
pub trait Planner { async fn plan(&self, goal, succeeded: &[(JobId, JobResult)]) -> PlanDelta; }
pub enum PlanDelta { Add(Vec<Job>), Done }
```

DAG 排空后,Orchestrator 带着**已完成结果**重新调用 Planner,它可以把新 Job 并进
正在跑的 DAG(`Add`)。**这正是"plan-then-execute"工作流缺失的反馈边**——有了它,
执行结果才能反过来改变计划,系统才从"工作流引擎"变成"agent 运行时"。

### 并发模型(关键取舍)

sub-agent future 是 `!Send`,所以**不用 `tokio::spawn`,用 `FuturesUnordered` 单线程
协作并发**——对 I/O 密集的 LLM 调用足够。每个 Job 从 `WorldFactory` 现造一个
`World`:既回避并发 Job 间 `&mut World` 的别名冲突,又给每个 Job worker 式隔离。
执行单元由 `JobRunner` trait 解耦,默认 `SubagentJobRunner`(一个 Job = 一个隔离 sub-agent)。

### 可靠性与治理

- **重试**:`RetryPolicy { max_attempts, Backoff::{None,Fixed,Exponential} }`;耗尽进
  `DeadLettered`,其下游被 `Cancelled`(死信阻断,不静默挂起)。
- **运行级 token 预算**:`RunBudget` 跨所有 Job 累计封顶,超了停 Run——补上多数异步
  编排方案漏掉的**成本闸门**。
- **DAG 环检测**:启动即拒绝有环的图,而非死锁。

> 例子见 `examples/orchestrator-demo`(3 路并发研究 + 1 路汇聚,跑真实模型)。
> 完整 API 见 crate 的 `lib.rs`。
