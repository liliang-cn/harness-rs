...
**Executed as `2168f58` (event order) + `b76bde4` (feed and trace); review fixes in the commit after.** The event-order test passed first time, as predicted. Code review found: (1) *plan defect* — the `tool_calls=1` assertion as written matched `model.complete`'s own `tool_calls=1` field (calls the model *requested*), so it passed with the whole `Cancelled` arm deleted; Step 8 above is corrected to scope the check to the `run.end` line; (2) a **hook-denied** tool call fires `PreToolUse` and never `PostToolUse`, so its `tool_starts` entry leaks for the hook's lifetime (pre-existing), and the new wholesale drain then bills that stale entry to a later run's cancel — fixed by clearing the per-call maps at `SessionStart` (bounds the leak to one run; the full fix, a terminal event on the deny path, is a follow-up), with a two-run regression test whose two calls deliberately differ in args because `MockModel` derives a call's id from name+args and identical calls would share a map key; (3) the module doc's event tree lacked `run.cancelled` (and `model.first_token`); (4) the `model_calls`/`tool_calls` asymmetry — completed calls versus dispatches — is right and now documented on the fields. Reviewer judgments kept as is: `warn` for `run.cancelled` (it is doing filter work, not severity work), one event-order path is enough proof, and the drain belongs in the `Cancelled` arm rather than `SessionEnd` because no other exit leaves a dangling entry (the deadline check is at the top of the iteration; the `Stuck` exits are after the tool loop).
...
**Task 6 executed as `b84fca2`. Gate run twice (before and after the final-review fix): fmt clean, clippy clean, workspace tests with exactly the three permitted doctest failures.** The final whole-branch review found no merge-blocker but one real hole the per-task reviews had all looked past: **`force_final_synthesis` was a bare `self.model.complete(ctx).await` that the token never raced**, reachable from both the monotony/`Stuck` exit and the budget-exhausted exit after the last cancel check — so a Ctrl-C landing there made one more full model call and returned `BudgetExhausted` with exit 0, while three docs (the `cancel` field, `model_step`, the changelog's "not even the forced final synthesis") promised the opposite. The existing test `cancellation_does_not_force_a_final_synthesis` pinned only a cancel that lands *before* the exit is chosen. Fixed in `45ba779`: the helper returns a private `Synthesis { Text(Option<String>), Cancelled }`, races the call in a `biased` select, restores the tool schemas and pops its own prompt turn on cancel, and both callers return `Outcome::Cancelled` after `Cancelled` + `SessionEnd`, skipping the learning review; new test `a_cancel_during_the_forced_synthesis_returns_cancelled` was red as `BudgetExhausted { last_text: Some("SYNTHESISED") }` first. Same commit: four "Esc" references became Ctrl-C (the CLI has no Esc binding; this plan's own text still says Esc in places and is left as the record it is), the changelog's "30th lifecycle event" count went (it was 32; the branch had already deleted the stale count from `event.rs` and then re-invented it one file over), the `translate_turn` empty-assistant-turn fix got its `### Fixed` line, and the `Outcome::Cancelled` doc now agrees with telemetry that a dropped dispatch still counts in `tools_called`. Cosmetic minors from the re-review in `bdbbe9a`. Re-review: ready to merge.

## Follow-ups found in review, deliberately out of scope for this branch

- **`SessionRecorder` drops `Event::Cancelled`** (`crates/harness-loop/src/replay.rs` ~179 and ~554, `_ => None`), so a recorded cancelled session replays as an ordinary `End`. Consistent with `Stop` (also unrecorded, and never fired), so not a regression — but it is the third `Event::Cancelled` consumer after `BroadcastHook` and `TelemetryHook`, and the only one left unwired. `hooks/audit.rs` ~281 writes its session-end record with no cancel marker either. Both belong with whatever gives `Event::Cancelled` fields.
- **Nothing tests the REPL's fresh-token-per-turn wiring itself** (`main.rs` ~864 `loop_.cancel = current.arm()`): `interrupt.rs`'s tests cover `Current`, not the call site, because `harness-cli` has no lib target. Moving `arm()` outside the turn loop would compile and pass every test.
