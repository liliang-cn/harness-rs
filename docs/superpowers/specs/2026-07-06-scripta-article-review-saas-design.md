...
**做什么**：用户把一篇文章导入系统，得到两类反馈——写作质量问题与 AI 味识别；
可以选中任意段/句/词/字下指令让 AI 定点改写，也可以接受全文扫描出的逐条建议；
每一次接受的修改进入真实 git 历史，可对比、可回滚、可看评分随版本的变化。
...
POST   /documents                新建（初始化 bare 仓库 + 首个 commit）
...
每次 `scan/rewrite/score` 落一条 `usage_events`，记 input/cached/output tokens
...
- **P0 引擎+git**：`scripta-engine` + gix 存储 + 文档/锚点模型，用集成测试对真实模型验证
  scan / rewrite / score / commit / diff 闭环（不碰前端）。
